...
import (
	...
	"github.com/kamylaep/algorithmsgo/quicksort"
)
...
func TestQuickSortWithSuccess(t *testing.T) {
	...
	for _, d := range data {
		actualOrdered := quicksort.Sort(d.unordered)
		isOrdered := reflect.DeepEqual(d.ordered, actualOrdered)
		if !isOrdered {
			t.Fatalf("Expected true but got false [%v, %v]", d.ordered, actualOrdered)
		}
	}
}
